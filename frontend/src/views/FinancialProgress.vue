...
const preview     = ref(null)   // parsed but not saved
...
const filteredWorks = computed(() => {
  ...
  if (!q) return works.value.slice(0, 20)
  return works.value
    .filter(w =>
...
function selectWork(w) {
  workSearch.value  = w.loa_number || w.name_of_work || ''
...
<template>
  <div class="bg-white rounded-2xl soft-shadow h-full w-full flex flex-col overflow-hidden">
    ...
    <div class="flex-shrink-0 px-8 pt-7 pb-5 border-b border-gray-100">
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p class="text-gray-400 text-sm font-medium">Upload railway bill PDFs to track payment progress per item.</p>
        </div>
      ...
      <div class="mt-4 relative max-w-xl">
        <div class="flex items-center bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3 focus-within:ring-2 focus-within:ring-[#1D5F5E]/20 focus-within:border-[#1D5F5E] focus-within:bg-white transition-all">
          ...
          <input
            ...
            type="text"
            placeholder="Search LOA number, work name, contractor…"
            class="flex-1 bg-transparent text-sm text-gray-800 placeholder-gray-400 outline-none"
          />
          <button v-if="selectedWork" @click="clearWork" class="ml-2 text-gray-400 hover:text-gray-600">
        ...
        <!-- Dropdown -->
        <div
          v-if="showDropdown && filteredWorks.length"
          class="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden"
        >
          <div
            v-for="w in filteredWorks"
            :key="w.id"
            @mousedown.prevent="selectWork(w)"
            class="px-5 py-3 cursor-pointer hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0"
          >
            <p class="text-sm font-semibold text-gray-800">{{ w.loa_number }}</p>
            <p class="text-xs text-gray-500 truncate">{{ w.name_of_work }}</p>
          </div>
        </div>
      </div>

      <!-- Selected work chip -->
      <div v-if="selectedWork" class="mt-3 flex items-center gap-2 flex-wrap">
        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#EEF4F3] text-[#1D5F5E] text-xs font-semibold">
          <div class="i-carbon-checkmark text-xs"></div>
          {{ selectedWork.loa_number }}
        </span>
        <span class="text-xs text-gray-400 truncate max-w-xs">{{ selectedWork.name_of_work }}</span>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto px-8 py-6 space-y-6">
      ...
      <!-- No work selected -->
      <div v-if="!selectedWork" class="flex flex-col items-center justify-center h-48 text-gray-400 gap-2">
        <div class="i-carbon-money text-4xl"></div>
        <p class="text-sm">Search and select a work to view financial progress.</p>
      </div>
                        ...
                        <div class="flex-1 h-2 rounded-full bg-gray-100 overflow-hidden">
                          <div
                            ...
                            :style="{
                              ...
                              background: progressColor(item.progress_pct),
                            }"
                          ></div>
                        </div>
                        <span
