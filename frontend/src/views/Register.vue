    ...
    <div class="w-full max-w-md">
      ...
      <div v-if="success" class="bg-light-surface dark:bg-[#1c1c1e] rounded-2xl soft-shadow p-8 text-center border border-gray-100 dark:border-[#3a3a3c]">
        <div class="w-14 h-14 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center mx-auto mb-4">
          <div class="i-carbon-checkmark text-2xl text-green-600 dark:text-green-400"></div>
        </div>
        <h2 class="text-lg font-bold text-gray-800 dark:text-white mb-2">Registration Submitted</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Your request is pending admin approval. You will be able to log in once approved.</p>
        <router-link to="/login" class="inline-block px-6 py-2.5 rounded-xl bg-[#1D5F5E] text-white text-sm font-semibold hover:bg-[#174E4D] transition-colors">
          Back to Login
        </router-link>
      </div>
      ...
      <div v-else class="bg-light-surface dark:bg-[#1c1c1e] rounded-2xl soft-shadow p-8 border border-gray-100 dark:border-[#3a3a3c]">
        <h1 class="text-xl font-bold text-gray-800 dark:text-white mb-1">Request Access</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-8">Fill in your details. Admin will approve your account.</p>

        <form @submit.prevent="handleRegister" class="flex flex-col gap-5">
          <div v-for="f in fields" :key="f.key" class="flex flex-col gap-1.5">
            <label class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">{{ f.label }}</label>
            <input
              v-model="form[f.key]"
              :type="f.type"
        ...
        <div class="mt-6 pt-6 border-t border-gray-200 dark:border-[#3a3a3c] text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">
            ...
            <router-link to="/login" class="font-semibold text-[#1D5F5E] hover:underline">Sign in</router-link>
          </p>
        </div>
      </div>
    </div>
