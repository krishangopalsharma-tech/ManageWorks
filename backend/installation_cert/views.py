...
def _auto_cert_number(work):
    """Auto-suggest cert number: {tender_number}/{seq:03d}"""
    seq = GeneratedCertificate.objects.filter(work=work).count() + 1
    base = work.tender_number or work.loa_number or str(work.id)
...
class GenerateCertView(APIView):
    ...
    def post(self, request):
        ...
        if not cert_number:
            cert_number = _auto_cert_number(work)
